class DbObject
  ...
  def self.get_records sql
    resrecs = Database.query sql      

    if resrecs && resrecs.count > 0  
      res = []
      resrecs.each do |r|
        res << hash_to_object(r)        
        Log.debug "Got record: #{r.uid} " if r.respond_to? :uid
        Log.debug "Got record: #{r.id} " if r.respond_to? :id
      end
      return res
    else
      Log.debug "No records found for : #{sql}"
      return []
    end
    nil
  end
  ...
  def self.find_all_by condition
    sql = "SELECT * from #{table_name} where #{condition}"      
