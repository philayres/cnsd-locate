...
describe SecureApi::ApiAuth do
  ...
  it "should get the secret for the client from the database" do
    ...
    expect(res.secret).to eq secret
  ...
  it "should return same one time token" do
    ...
    expect(ot['X-Nonce']).to_not be_nil
    ...
    expect(['X-Nonce']).to_not eq ot_test['X-Nonce']
    ...
    expect(secret_new).to eq secret
